...
\begin{frame}{example}
	...
	\only<1>{
		\begin{align*}
			\varphi &\equiv \neg(\Box\Diamond p \to \Box(\neg p \to \Diamond r))\\
					&\equiv \Box\Diamond p \land \neg\Box(\neg p \to \Diamond r)\\
					&\equiv \Box\Diamond p \land \Diamond\neg(p \lor \Diamond r)\\
					&\equiv \Box\Diamond p \land \Diamond(\neg p \land \Box\neg r)
		\end{align*}
	}
	\only<2>{
...
initial text=$ $, % sets the text that appears on the start arrow
	...
	\begin{adjustbox}{max width=.95\textwidth, max totalheight=.95\textheight}
		\begin{tikzpicture}
			...
			\node[state, initial, accepting, left = of a] (1) {$\langle \{\ell_{2}, \ell_{4}\}, \emptyset \rangle$};
...
\begin{frame}{Illustration of $\mathcal{F}'_{\phi}$}
	...
	\begin{figure}[ht]
		...
		\begin{tikzpicture}
			...
			\only<2->{
				\tikzset{
					every state/.style={thick, fill=yellow!50, scale = 0.6}
				}
			}
			\only<2-3>{
				\node[state, initial] (ini) at (-5, 0) {};
			...
			\only<3>{
				\node[state, initial, fill = gray!10] (ini) at (-5, 0) {};
			}
			\only<4-7>{
				\node[state, accepting] (t3) at (-2.8, 3.2) {t3};
				\node[state, accepting] (t5) at (3.6, 2) {t5};
...
\begin{frame}{Illustration of $\mathcal{F}'_{\phi}$ (another example)}
	\tikzset{
		%->, % makes the edges directed
		every cross out node/.append style={-,solid},
		>=stealth, % makes the arrow heads bold
	...
	\begin{figure}[ht]
		...
		\begin{tikzpicture}
			...
			\node[state] (t2) at (-2, 2) {t2};
			\node[state] (t1) at (-3.6, 2) {t1};
			\node[state] (t3) at (-2.8, 3.2) {t3};

			\node[state] (t4) at (2, 2) {t4};
			\node[state] (t5) at (3.6, 2) {t5};
			\node[state] (t6) at (2.8, 3.2) {t6};

			\node[state] (b2) at (-2, -2) {b2};
			\node[state] (b1) at (-3.6, -2) {b1};
			\node[state] (b3) at (-2.8, -3.2) {b3};
			...
			\only<2->{
				\tikzset{
					every state/.style={thick, fill=blue!50, scale = 0.6}
				}
			}
			\only<2-3>{
				\node[state, initial] (ini) at (-5, 0) {};
...
initial text=$ $, % sets the text that appears on the start arrow
	...
	\begin{figure}[ht] % ’ht’ tells LaTeX to place the figure ’here’ or at the top of the page
	\centering % centers the figure
	\begin{adjustbox}{max width=0.8\textwidth, max totalheight=\textheight}
		\begin{tikzpicture}
			\node[] (then) {then};
...
\begin{frame}{Simulation(cont'd)}
	...
	\begin{figure}[ht]
		...
		\begin{tikzpicture}
			...
			\only<1>{
				\node[fill = white, right = 0.2cm of q2, minimum size=2cm] (?) {$\subseteq \text{Post}(\mathnormal{x})$?};
			}
