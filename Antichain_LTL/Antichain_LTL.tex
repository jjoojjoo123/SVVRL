...
\begin{frame}{Miyano-Hayashi construction example}
\tikzset{
->, % makes the edges directed
>=stealth, % makes the arrow heads bold
every edge/.style={draw, thick, black},
...
initial text=$ $, % sets the text that appears on the start arrow
	...
	\begin{figure}[ht] % ’ht’ tells LaTeX to place the figure ’here’ or at the top of the page
	\centering % centers the figure
	\begin{adjustbox}{max width=0.8\textwidth, max totalheight=0.8\textheight}
		\begin{tikzpicture}
			\node[state, initial] (a) {$\langle \{\ell_{2}, \ell_{4}\}, \{\ell_{2}\}\rangle$};
			...
			\draw (a) edge[loop above] node[left]{$\{p\}, \{p, r\}$} (a);
			\draw (a) edge[bend left = 15] node[above = 0.5cm]{$\emptyset, \{p\}, \{r\}, \{p, r\}$} (b);
			\draw (b) edge[bend left = 15] node[below = 0.3cm]{$\{p\}, \{p, r\}$} (a);
			\draw (b) edge[loop above] node{$\emptyset, \{p\}, \{r\}, \{p, r\}$} (b);
			\draw (b) edge node[right]{$\emptyset$} (c);
			\draw (a) edge node[below]{$\emptyset$} (c);
			\draw (c) edge[loop below] node{$\emptyset, \{p\}$} (c);
			...
			\draw (e) edge[loop above] node{$\emptyset, \{p\}$} (e);
		\end{tikzpicture}
	\end{adjustbox}
...
\begin{frame}{Closed Sets and Antichains}
	\begin{itemize}
		\item Let $\preceq \subseteq Q \times Q$.
...
\begin{frame}{Closed Sets and Antichains(cont'd)}
	\begin{itemize}
...
initial text=$ $, % sets the text that appears on the start arrow
	...
	\begin{adjustbox}{max width=\textwidth, max totalheight=\textheight}
		\begin{tikzpicture}
			\node[] (then) {then};
			\node[state, above left = of then] (l2) {$\ell_{2}$};
			\node[state, left = of l2] (l1) {$\ell_{1}$};
			\node[state, below = of l1] (l3) {$\ell_{3}$};
			\node[below left = of l1] (if) {If};
			
			\node[state, below right = of then] (l3') {$\ell_{3}$};
			\node[state, right = of l3'] (l4') {$\ell_{4}$};
			\node[state, above = of l4'] (l2') {$\ell_{2}$};
		\end{tikzpicture}
	\end{adjustbox}
		\caption{Simulation}
	\end{figure}
