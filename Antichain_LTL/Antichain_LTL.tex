...
initial text=$ $, % sets the text that appears on the start arrow
	...
	\begin{figure}[ht] % ’ht’ tells LaTeX to place the figure ’here’ or at the top of the page
	\centering % centers the figure
	\begin{adjustbox}{max width=0.4\textwidth, max totalheight=0.4\textheight}
		\begin{tikzpicture}
			\node[] (if) {If};
			\node[state, above right = 1cm and of if] (l1) {$\ell_{1}$};
			\node[state, right = of l1] (l2) {$\ell_{2}$};
			\node[state, below right = 1cm and of if] (l3) {$\ell_{3}$};
		\end{tikzpicture}
	\end{adjustbox}
		\caption{Simulation}
	\end{figure}
