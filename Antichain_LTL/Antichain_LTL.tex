...
\begin{frame}{Linear Temporal Logic(cont'd)}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item or $\phi \equiv \neg\phi_{1}$ and $\mathnormal{w} \models \phi_{1}$,
			\item or $\phi \equiv \phi_{1} \lor \phi_{2}$ and $\mathnormal{w} \models \phi_{1}$ or $\mathnormal{w} \models \phi_{2}$,
...
$\sigma_{i}\sigma_{i+1}\cdots \models \phi_{1}$.
		...
		\item let $\Diamond\phi = \text{true} \mathcal{U}\phi$, $\Box\phi =
