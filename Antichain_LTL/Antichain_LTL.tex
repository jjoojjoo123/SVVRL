...
\begin{frame}{Simulation}
	\begin{itemize}
		...
		\item Recall $\mathcal{A}^{-1} = \langle \text{Loc}, I, \Sigma, \delta^{-1}, \alpha \rangle$ where $\delta^{-1} = \{(\ell, \sigma, \ell') \mid (\ell', \sigma, \ell) \in \delta\}$.
...
% 		$$\mathcal{R}_{\alpha}^{\mathcal{K}} \equiv \alpha' \cap \mu x \cdot (\text{Post}_{\text{MC}}(x) \cup I~')$$
% 		$$\mathcal{F}_{\phi}^{\mathcal{K}} \equiv \nu y \cdot \mu x \cdot (\text{Post}_{\text{MC}}(x) \cup (\text{Post}_{\text{MC}}(y) \cap \mathcal{R}_{\alpha}^{\mathcal{K}}))$$
% 	\end{itemize}
% \end{frame}
