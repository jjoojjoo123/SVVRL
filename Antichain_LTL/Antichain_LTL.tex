...
\begin{frame}{Simulation(cont'd)}
	\begin{itemize}
		\item Given a backward-simulation $\succeq$ for $\text{MH}(\mathcal{A}_{\phi})$, all the sets that are computed to evaluate $\mathcal{R}_{a}$ and $\mathcal{F}'_{\phi}$ are $\succeq$-closed, that is $I^{\text{MH}}$ and $\alpha^{\text{MH}}$ are $\succeq$-closed, and $x \cap y$, $x \cup y$ and $\text{Pre}(x)$ are $\succeq$-closed whenever $x$ and $y$ are $\succeq$-closed.
		\item Recall
...
\begin{frame}{Semi-symbolic Algorithm for $\text{Post}(\cdot)$}
	\begin{itemize}
		\item When computing the successors of a pair $\langle s, o \rangle$, the algorithm uses the intermediate boolean variables $\mathnormal{x}_{1},\cdots , \mathnormal{x}_\mathnormal{n}, \mathnormal{y}_{1},\cdots , \mathnormal{y}_\mathnormal{n}$ and $\mathnormal{y}~'_{1},\cdots , \mathnormal{y}~'_\mathnormal{n}$ to encode respectively the sets $s', o' \setminus  \alpha$ and $o'$ where $\langle s', o' \setminus \alpha \rangle \in \text{Post}(\llbracket \{ \langle s, o \rangle \} \rrbracket_{\succeq_{\text{alt}}})$.
